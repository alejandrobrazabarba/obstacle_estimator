...
#include <obstacle_estimator/centralized_estimator.h>
#include <obstacle_estimator/candidate.h>
#include <obstacle_estimator/CandidateMsg.h>
...
protected:
	...
	/// Callbacks
	void candidatesReceived(const obstacle_estimator::CandidateMsg::ConstPtr& candidate_msg);
...
EstimatorNode::EstimatorNode()
{
	...
	string candidate_topic_name = "candidates";
	candidate_sub_ = nh_->subscribe<obstacle_estimator::CandidateMsg>(candidate_topic_name.c_str(), 1, &EstimatorNode::candidatesReceived, this);
...
*/
void EstimatorNode::candidatesReceived(const obstacle_estimator::CandidateMsg::ConstPtr& candidate_msg)
{
	...
	{		
		...
		for(int i = 0; i < 2; i++)
		{
			for(int j = 0; j < 2; j++)
			{
				cand_p->location_covariance(i,j) = candidate_msg->location_covariance[i*3+j];
				cand_p->speed_covariance(i,j) = candidate_msg->speed_covariance[i*3+j];
			}	
		}	
